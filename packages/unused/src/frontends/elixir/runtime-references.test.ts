...
function withExactPrivateModuleScaffolding(root: string, trace: TraceResult): TraceResult {
  const moduleEvents: TraceEvent[] = [];
  for (const owner of trace.modules) {
    const lines = readFileSync(join(root, owner.file), "utf8").split("\n");
    moduleEvents.push({
      k: "event",
      kind: "remote",
      file: owner.file,
    ...
    for (let index = 0; index < lines.length; index += 1) {
      const definition = /^\s*(defp?)\s+[a-z_][A-Za-z0-9_]*[!?]?(?=\s|\(|,)/u.exec(
        lines[index] ?? "",
      )?.[1];
      if (definition !== "def" && definition !== "defp") continue;
      const line = index + 1;
      moduleEvents.push(
        {
          k: "event",
...
describe("extractElixirRuntimeReferences", () => {
  ...
  it("summarizes exact same-module public parameters without summarizing public results", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-flow-"));
    const file = "public_atom_flow.ex";
    const moduleName = "NeutralPublic.Flow";
    const lines = [
      `defmodule ${moduleName} do`,
      "  def safe(map, raw), do: consume(map, String.to_atom(raw))",
      "  def pass(map, raw), do: Map.has_key?(map, identity(String.to_atom(raw)))",
      "  def invoke(raw), do: dispatch(String.to_atom(raw))",
      "  def escaped(raw), do: identity(String.to_atom(raw))",
      "  def produced(raw), do: make(raw)",
      "  def consume(map, key), do: Map.has_key?(map, key)",
      "  def identity(value), do: value",
      "  def dispatch(key), do: apply(NeutralPublic.Target, key, [])",
    ...
    const local = (line: number, fromFun: string, name: string, arity: number): TraceEvent =>
      event(line, fromFun, moduleName, name, arity, { kind: "local" });
    const atom = (line: number, fromFun: string): TraceEvent =>
      event(line, fromFun, "String", "to_atom", 1, { dyn: true });
    const functions = lines.flatMap((line, index) => {
      const match = /^\s*def\s+([a-z_][A-Za-z0-9_]*[!?]?)\(([^)]*)\)/u.exec(line);
    ...
    const trace: TraceResult = {
      ...
      events: [
        atom(2, "safe/2"),
        local(2, "safe/2", "consume", 2),
        atom(3, "pass/2"),
        event(3, "pass/2", "Map", "has_key?", 2),
        local(3, "pass/2", "identity", 1),
        atom(4, "invoke/1"),
        local(4, "invoke/1", "dispatch", 1),
        atom(5, "escaped/1"),
        local(5, "escaped/1", "identity", 1),
        local(6, "produced/1", "make", 1),
        event(7, "consume/2", "Map", "has_key?", 2),
        event(9, "dispatch/1", "Kernel", "apply", 3, { dyn: true }),
        atom(10, "make/1"),
      ],
    };
    ...
    try {
      ...
      expect(
        ...
        { fromFun: "safe/2", flow: "data" },
        { fromFun: "pass/2", flow: "data" },
        { fromFun: "invoke/1", flow: "delegated-invocation" },
        { fromFun: "escaped/1", flow: "escape" },
        { fromFun: "make/1", flow: "escape" },
      ]);
      expect(extraction.atomFlowStats).toMatchObject({
        privateFunctions: 0,
        privateSummaries: 0,
        privateCallEdges: 0,
        publicSummaryFunctions: 9,
        publicSummaries: 12,
        publicCallEdges: 5,
        publicSummaryMatches: 4,
        publicOpaqueFunctions: 0,
        joinedProducerOutcomes: 5,
        dataSinks: 2,
        invocationSinks: 1,
        escapes: 2,
      });
      expect(extraction.atomFlowStats.publicSccIterations).toBeGreaterThan(0);
      expect(extraction.atomFlowStats.publicSummaryUpdates).toBeGreaterThan(0);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });

  it("summarizes an exact remote public parameter across project-owned module files", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-cross-file-"));
    const callerFile = "cross_file_caller.ex";
    const targetFile = "cross_file_target.ex";
    const callerModule = "NeutralCrossFile.Caller";
    const targetModule = "NeutralCrossFile.Target";
    const callerLines = [
      `defmodule ${callerModule} do`,
      `  def safe(map, raw), do: ${targetModule}.consume(map, String.to_atom(raw))`,
      "end",
      "",
    ];
    const targetLines = [
    ...
    const trace: TraceResult = {
      ...
      functions: [
        { ...fn(callerModule, "safe", 2, callerFile), line: 2 },
        { ...fn(targetModule, "consume", 2, targetFile), line: 2 },
      ],
      events: [
        {
          k: "event",
    ...
    try {
      ...
      writeFileSync(join(root, targetFile), targetLines.join("\n"));
      const extraction = extractElixirRuntimeConventions(
        root,
        withExactPrivateModuleScaffolding(root, trace),
      );
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        publicSummaryFunctions: 2,
        publicCallEdges: 0,
        publicSummaryMatches: 0,
        crossModuleSummaryFunctions: 2,
        crossModuleCallEdges: 1,
        crossModuleSummaryMatches: 1,
  ...
  it.each([
    ...
    ["default", "def helper(value \\\\ :fallback), do: Atom.to_string(value)"],
    ["pattern", "def helper({:ok, value}), do: Atom.to_string(value)"],
    [
      "multiple-clause",
      "def helper(value), do: Atom.to_string(value)\n  def helper(value), do: value.run()",
    ],
  ])("keeps a public %s definition outside parameter summaries", (_label, definition) => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-unsupported-"));
    const file = "public_atom_unsupported.ex";
    const moduleName = "NeutralPublic.Unsupported";
    const lines = [
      `defmodule ${moduleName} do`,
    ...
    const trace: TraceResult = {
      ...
      modules: [mod(moduleName, file)],
      functions: [
        { ...fn(moduleName, "run", 1, file), line: 2 },
        { ...fn(moduleName, "helper", 1, file), line: 3 },
      ],
      events: [
        {
          k: "event",
  ...
  it("requires one exact reflected public definition at the source line", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-reflection-"));
    const file = "public_atom_reflection.ex";
    const moduleName = "NeutralPublic.Reflection";
    const lines = [
      `defmodule ${moduleName} do`,
      "  def run(raw), do: helper(String.to_atom(raw))",
      "  def helper(value), do: Atom.to_string(value)",
    ...
    const runFunction = { ...fn(moduleName, "run", 1, file), line: 2 };
    const helperFunction = { ...fn(moduleName, "helper", 1, file), line: 3 };
    const trace: TraceResult = {
      appMod: null,
      deps: [],
      compileOk: true,
      testPartition: "complete",
      modules: [mod(moduleName, file)],
      functions: [runFunction, helperFunction],
      events: [
        {
          k: "event",
          kind: "remote",
    ...
    try {
      writeFileSync(join(root, file), lines.join("\n"));
      const exact = extractElixirRuntimeConventions(
        root,
        withExactPrivateModuleScaffolding(root, trace),
      );
      expect(
        exact.dynamicDispatches.find((fact) => fact.factKind === "computed-atom"),
      ).toMatchObject({ flow: "data" });
      expect(exact.atomFlowStats.publicSummaryFunctions).toBe(2);

      for (const functions of [
        [runFunction],
        [runFunction, { ...helperFunction, line: 4 }],
        [runFunction, helperFunction, { ...helperFunction }],
      ]) {
        const rejected = extractElixirRuntimeConventions(
          root,
          withExactPrivateModuleScaffolding(root, { ...trace, functions }),
        );
        expect(
          rejected.dynamicDispatches.find((fact) => fact.factKind === "computed-atom"),
        ).toMatchObject({ flow: "escape" });
        expect(rejected.atomFlowStats).toMatchObject({
          publicSummaryFunctions: 1,
          publicCallEdges: 0,
          publicSummaryMatches: 0,
        });
  ...
  it("accepts an otherwise exact public summary target across modules", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-cross-module-"));
    const callerFile = "public_atom_cross_module.ex";
    const targetFile = "public_atom_cross_target.ex";
    const callerModule = "NeutralPublic.CrossModule";
    const targetModule = "NeutralPublic.CrossTarget";
    const callerLines = [
      `defmodule ${callerModule} do`,
      `  def run(raw), do: ${targetModule}.helper(String.to_atom(raw))`,
      "end",
      "",
    ];
    const targetLines = [
    ...
    const trace: TraceResult = {
      ...
      functions: [
        ...
        { ...fn(targetModule, "helper", 1, targetFile), line: 2 },
      ],
      events: [
        {
          k: "event",
  ...
  it("rejects a same-module public summary call across partitions", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-call-world-"));
    const file = "public_atom_call_world.ex";
    const moduleName = "NeutralPublic.CallWorld";
    const lines = [
      `defmodule ${moduleName} do`,
      "  def run(raw), do: helper(String.to_atom(raw))",
      "  def helper(value), do: Atom.to_string(value)",
  ...
  it("rejects modules with an unreviewed module-level definition expansion", () => {
    ...
    try {
      ...
      expect(extraction.atomFlowStats.privateModuleUnknownEventRejections).toBe(1);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it.each([
    ...
    const moduleName = "NeutralPrivate.ModuleConstruct";
    const constructLines = construct.split("\n");
    const runLine = constructLines.length + 2;
    const helperLine = runLine + 1;
    const trace: TraceResult = {
      appMod: null,
      deps: [],
      compileOk: true,
  ...
  it("rejects an incomplete metadata compiler bundle despite an inert callee name", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-private-module-metadata-partial-"));
    const file = "private_module_metadata_partial.ex";
    const moduleName = "NeutralPrivate.MetadataPartial";
    const trace: TraceResult = {
      appMod: null,
      deps: [],
      compileOk: true,
  ...
  it("rejects a production module when every module-level compiler event is absent", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-private-module-events-absent-"));
    const file = "private_module_events_absent.ex";
    const moduleName = "NeutralPrivate.EventsAbsent";
    const trace: TraceResult = {
      appMod: null,
      deps: [],
      compileOk: true,
  ...
  it("keeps same-module public summary work linear from 250 through 1,000 functions", () => {
    const run = (count: number) => {
      const root = mkdtempSync(join(tmpdir(), `unused-public-atom-scale-${count}-`));
      const file = "public_atom_scale.ex";
      const moduleName = "NeutralPublic.Scale";
      const lines = [
        `defmodule ${moduleName} do`,
        "  def safe(map, raw), do: Map.has_key?(map, step_0(String.to_atom(raw)))",
      ];
      const functions: FunctionRecord[] = [{ ...fn(moduleName, "safe", 2, file), line: 2 }];
      const events: TraceEvent[] = [
        {
          k: "event",
          kind: "remote",
      ...
      for (let index = 0; index < count; index += 1) {
        const line = index + 3;
        const name = `step_${index}`;
        functions.push({ ...fn(moduleName, name, 1, file), line });
        if (index + 1 === count) {
          lines.push(`  def ${name}(value), do: value`);
          continue;
        }
        const target = `step_${index + 1}`;
        lines.push(`  def ${name}(value), do: ${target}(value)`);
        events.push({
          k: "event",
          kind: "local",
          file,
    ...
    for (const count of [250, 500, 1_000]) {
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        ...
        privateSummaries: 0,
        privateCallEdges: 0,
        publicSummaryFunctions: count + 1,
        publicSummaries: count + 2,
        publicCallEdges: count,
      ...
      expect(extraction.atomFlowStats.publicSccIterations).toBeLessThanOrEqual((count + 1) * 4);
      expect(extraction.atomFlowStats.publicSummaryUpdates).toBeLessThanOrEqual((count + 2) * 3);
      expect(extraction.atomFlowStats.roleEdges).toBeLessThanOrEqual(count * 7 + 20);
      expect(extraction.atomFlowStats.queueVisits).toBeLessThanOrEqual(count * 18 + 30);
    }
  });

  it("keeps cross-module public summary work linear from 250 through 1,000 files", () => {
    const run = (count: number) => {
      const root = mkdtempSync(join(tmpdir(), `unused-cross-module-atom-scale-${count}-`));
      const entryFile = "cross_scale_entry.ex";
      const entryModule = "NeutralCrossScale.Entry";
      const modules: ModuleRecord[] = [mod(entryModule, entryFile)];
      const functions: FunctionRecord[] = [{ ...fn(entryModule, "safe", 2, entryFile), line: 2 }];
      const events: TraceEvent[] = [
        {
          k: "event",
          kind: "remote",
        ...
        },
        {
          ...
          from_mod: entryModule,
          from_fun: "safe/2",
          to_mod: "NeutralCrossScale.Step0",
          name: "step_0",
          arity: 1,
          dyn: false,
          partition: "prod",
      ...
      try {
        writeFileSync(
          ...
          [
            ...
            "  def safe(map, raw), do: Map.has_key?(map, NeutralCrossScale.Step0.step_0(String.to_atom(raw)))",
            "end",
            "",
          ].join("\n"),
        );
        for (let index = 0; index < count; index += 1) {
          const file = `cross_scale_step_${index}.ex`;
          const moduleName = `NeutralCrossScale.Step${index}`;
          const nextModule = `NeutralCrossScale.Step${index + 1}`;
          const functionName = `step_${index}`;
          const nextFunction = `step_${index + 1}`;
          modules.push(mod(moduleName, file));
          functions.push({ ...fn(moduleName, functionName, 1, file), line: 2 });
          writeFileSync(
            join(root, file),
            [
              `defmodule ${moduleName} do`,
              index + 1 === count
                ? `  def ${functionName}(value), do: value`
                : `  def ${functionName}(value), do: ${nextModule}.${nextFunction}(value)`,
              "end",
              "",
            ].join("\n"),
          );
    ...
    for (const count of [250, 500, 1_000]) {
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        ...
        publicCallEdges: 0,
        publicSummaryMatches: 0,
        crossModuleSummaryFunctions: count + 1,
        crossModuleCallEdges: count,
        crossModuleSummaryMatches: 1,
        crossModuleCanonicalIdentityRejections: 0,
        crossModuleBoundaryEscapes: 0,
        crossModuleOpaqueFunctions: 0,
      ...
      expect(extraction.atomFlowStats.crossModuleSummaryUpdates).toBeLessThanOrEqual(
        (count + 2) * 3,
      );
  ...
  it("fails closed when a cross-module public summary exceeds the 64-callee bound", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-cross-module-atom-bound-"));
    const entryFile = "cross_bound_entry.ex";
    const hubFile = "cross_bound_hub.ex";
    const entryModule = "NeutralCrossBound.Entry";
    const hubModule = "NeutralCrossBound.Hub";
    const leafCount = 65;
    const modules: ModuleRecord[] = [mod(entryModule, entryFile), mod(hubModule, hubFile)];
    const functions: FunctionRecord[] = [
      { ...fn(entryModule, "run", 1, entryFile), line: 2 },
      { ...fn(hubModule, "hub", 1, hubFile), line: 2 },
    ];
    const events: TraceEvent[] = [
      {
        k: "event",
    ...
    const leafCalls = Array.from(
      { length: leafCount },
      (_, index) => `NeutralCrossBound.Leaf${index}.leaf_${index}(value)`,
    );
    try {
      writeFileSync(
        join(root, entryFile),
        [
          `defmodule ${entryModule} do`,
          `  def run(raw), do: ${hubModule}.hub(String.to_atom(raw))`,
          "end",
          "",
        ].join("\n"),
      );
      writeFileSync(
        join(root, hubFile),
        [
          `defmodule ${hubModule} do`,
          `  def hub(value), do: {${leafCalls.join(", ")}}`,
          "end",
          "",
        ].join("\n"),
      );
      for (let index = 0; index < leafCount; index += 1) {
        const file = `cross_bound_leaf_${index}.ex`;
        const moduleName = `NeutralCrossBound.Leaf${index}`;
        const functionName = `leaf_${index}`;
        modules.push(mod(moduleName, file));
        functions.push({ ...fn(moduleName, functionName, 1, file), line: 2 });
        writeFileSync(
          join(root, file),
          [
            `defmodule ${moduleName} do`,
            `  def ${functionName}(value), do: Atom.to_string(value)`,
            "end",
            "",
          ].join("\n"),
        );
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        crossModuleSummaryFunctions: leafCount + 2,
        crossModuleCallEdges: leafCount + 1,
        crossModuleSummaryMatches: 1,
        crossModuleOpaqueFunctions: 1,
        dataSinks: 0,
        escapes: 1,
      });
    } finally {
  ...
  it.each([
    ["terminal-bearing", true, "data"],
    ["terminal-free", false, "escape"],
  ] as const)("solves a %s cross-module public cycle", (_label, terminal, expectedFlow) => {
    const root = mkdtempSync(join(tmpdir(), "unused-cross-module-atom-cycle-"));
    const entryFile = "cross_cycle_entry.ex";
    const leftFile = "cross_cycle_left.ex";
    const rightFile = "cross_cycle_right.ex";
    const entryModule = "NeutralCrossCycle.Entry";
    const leftModule = "NeutralCrossCycle.Left";
    const rightModule = "NeutralCrossCycle.Right";
    const trace: TraceResult = {
      appMod: null,
      deps: [],
      compileOk: true,
      testPartition: "complete",
      modules: [
        mod(entryModule, entryFile),
        mod(leftModule, leftFile),
        mod(rightModule, rightFile),
      ],
      functions: [
        { ...fn(entryModule, "run", 2, entryFile), line: 2 },
        { ...fn(leftModule, "left", 1, leftFile), line: 2 },
        { ...fn(rightModule, "right", 1, rightFile), line: 2 },
      ],
      events: [
        {
          k: "event",
    ...
    try {
      writeFileSync(
        ...
        [
          `defmodule ${entryModule} do`,
          `  def run(map, raw), do: Map.has_key?(map, ${leftModule}.left(String.to_atom(raw)))`,
          "end",
          "",
        ].join("\n"),
      );
      writeFileSync(
        join(root, leftFile),
        [
          `defmodule ${leftModule} do`,
          `  def left(value), do: ${rightModule}.right(value)`,
          "end",
          "",
        ].join("\n"),
      );
      writeFileSync(
        join(root, rightFile),
        [
          `defmodule ${rightModule} do`,
          terminal
            ? `  def right(value), do: {${leftModule}.left(value), Atom.to_string(value)}`
            : `  def right(value), do: ${leftModule}.left(value)`,
          "end",
          "",
        ].join("\n"),
      );
      ...
      expect(
        extraction.dynamicDispatches.find((fact) => fact.factKind === "computed-atom"),
      ).toMatchObject({ fromFun: "run/2", flow: expectedFlow });
      expect(extraction.atomFlowStats).toMatchObject({
        crossModuleSummaryFunctions: 3,
  ...
  it("fails closed when a public summary exceeds the 64-callee bound", () => {
    const root = mkdtempSync(join(tmpdir(), "unused-public-atom-bound-"));
    const file = "public_atom_bound.ex";
    const moduleName = "NeutralPublic.Bound";
    const leafCount = 65;
    const leafCalls = Array.from({ length: leafCount }, (_, index) => `leaf_${index}(value)`);
    const lines = [
      `defmodule ${moduleName} do`,
      ...
      ...Array.from(
        { length: leafCount },
        (_, index) => `  def leaf_${index}(value), do: Atom.to_string(value)`,
      ),
      "end",
      "",
    ];
    const functions: FunctionRecord[] = [
      { ...fn(moduleName, "run", 1, file), line: 2 },
      { ...fn(moduleName, "hub", 1, file), line: 3 },
      ...Array.from({ length: leafCount }, (_, index) => ({
        ...fn(moduleName, `leaf_${index}`, 1, file),
        line: index + 4,
      })),
    ];
    const events: TraceEvent[] = [
    ...
    try {
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        publicSummaryFunctions: leafCount + 2,
        publicSummaries: leafCount + 2,
        publicCallEdges: leafCount + 1,
        publicSummaryMatches: 1,
        publicOpaqueFunctions: 1,
      });
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  ...
  it("keeps exact inert module-scope classification linear from 250 through 1,000 functions", () => {
    const run = (count: number) => {
      ...
      const metadataEvents = (line: number): TraceEvent[] => [
        moduleEvent(line, "alias", "Module"),
        moduleEvent(line, "imported", "Kernel", "@", 1),
        moduleEvent(line, "remote", "Module", "__put_attribute__", 5),
      ];
      const typespecEvents = (line: number): TraceEvent[] => [
        moduleEvent(line, "alias", "Kernel.Typespec"),
        moduleEvent(line, "imported", "Kernel", "@", 1),
  ...
  it("solves private summaries independently in production and test worlds", () => {
    ...
    try {
      ...
      expect(extraction.atomFlowStats).toMatchObject({
        ...
        privateSummaries: 4,
        privateCallEdges: 2,
        privateModuleScaffoldingEvents: 7,
        privateModuleAmbiguousEventRejections: 0,
