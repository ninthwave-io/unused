 ...
 */
export const ASSUMPTION_SET_VERSION = "1.21.0";
...
export const GLOBAL_ASSUMPTIONS: readonly GlobalAssumption[] = [
  ...
  },
  {
    ...
    detail:
      "ADR 0014 Phase 1B1 supersedes the older closed source-shape list in the preceding assumption while retaining every legacy exact-safe shape as a compatibility terminal. Compiler-confirmed `String.to_atom/1` and `String.to_existing_atom/1` values are followed within one function through indexed parentheses/containers, simple assignments, `with` bindings, pipelines, literal atom allowlists, and exact literal callback-result expressions. All producers on an exact carrier/partition share one value-role graph; a finite bitmask fixed point processes each unique edge a bounded number of times rather than rescanning shared assignment uses. A sparse validated language registry classifies exact public `Map`, `Keyword`, `MapSet`, `Atom`, and `Enum` argument/result roles. Explicit and piped `Enum.reduce/3` propagate exact callback results, while enumerable and accumulator roles remain omitted and fail closed because callback-input bindings are not summarized. The position-stable delimiter pass nests literal `fn ... end` blocks relative to their enclosing call, so multi-argument, multi-clause, and nested callback commas do not inflate logical arity; `fn:` remains an ordinary keyword key. The registered built-in `convention:ecto` plugin owns an intentionally small typed `Ecto.Changeset` and `Ecto.Type` provider, enabled before graph emission only with the `ecto` dependency and no project-owned module spoof. Every summarized call requires unique source cardinality and one canonical compiler event on the same carrier/partition, so aliases and imports resolve through compiler identity while production and test joins remain distinct. Project-owned core lookalikes, omitted argument roles, unknown calls or returns, rebinding, interpolation, ambiguous cardinality, callback ambiguity, captures, MFA selectors, `apply`, and computed receivers fail closed as computed-value escape or invocation. Atom-to-string conversion is terminal non-executable data. Returned or stored collections, including replacement/update keys, propagate their computed atom rather than consuming it. This phase has no same-module or inter-module function summaries and changes no public JSON schema. Indexed counters include joined outcomes, unjoined opaque fallbacks, and legacy/indexed disagreements; event-populated general and callback-heavy 250-to-1,000-function fixtures plus a dense P-producer/U-use fixture guard bounded local scaling.",
  },
  {
