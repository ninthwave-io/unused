 ...
 */
export const ASSUMPTION_SET_VERSION = "1.9.0";
...
export const GLOBAL_ASSUMPTIONS: readonly GlobalAssumption[] = [
  ...
  },
  {
    ...
    detail:
      "Elixir test files and effective test-only compiler paths are traced in a separate child with `MIX_ENV=test`, `--no-start`, and a distinct isolated build. The analyzer starts ExUnit with `autorun: false` only to compile deterministically sorted `_test.exs` sources; it does not run `mix test`, require `test/test_helper.exs`, or start the analyzed application. Test facts are accepted only from explicit test files and the non-production `elixirc_paths` delta. Compatible production re-emission is discarded; a novel production-environment function or edge, conflicting ownership, malformed/truncated protocol, missing matching dependency artifact, timeout, runtime exit, or support/test/reflection failure discards every test fact and makes the partition incomplete. The boundary is then published as `status: partial` with `partitions.test: incomplete`, a deterministic warning is written to stderr, and a conservative safety root keeps every compiler-known production file, module, and public function alive. Exact outgoing bridge edges remain reachable too. No potentially test-reachable subject receives an unused/test-only claim or a supported deletion plan until the test partition completes; unrelated language boundaries remain analyzable.",
  },
];
