 ...
 */
export const ASSUMPTION_SET_VERSION = "1.28.0";
...
export const GLOBAL_ASSUMPTIONS: readonly GlobalAssumption[] = [
  ...
  {
    id: "elixir-default-argument-wrapper-reachability",
    title: "Elixir default-argument wrappers retain their declared body",
    detail:
      "Compiler-authoritative EEP-48 default counts map every generated lower-arity wrapper to its declared maximum-arity body; when documentation is deliberately disabled, the same fact must be recovered from the Elixir debug-info definition table. The exact wrapper-to-body edge is one-way, so a short call retains the body it executes while a direct maximum-arity call does not retain unused wrappers. Missing documentation still retains an unambiguous export surface with line-zero evidence, but ambiguous absent default metadata, malformed or non-contiguous wrapper families, cross-file or cross-partition targets, and target/source-line disagreement fail the production phase closed or make only the test partition incomplete.",
  },
  {
    id: "elixir-structural-protocol-v2-has-closed-runtime-consumers",
    title: "Elixir structural protocol v2 has closed, bounded runtime consumers",
    detail:
      "The isolated compiler child attempts one bounded read of every compiler-owned Elixir source and, when the file is readable and within cap, parses it once with `Code.string_to_quoted/2` and emits a source-minimized, bounded internal bundle: repository-relative owner path, content digest and byte count, bounded visit/depth counters, module/function carrier identities, grapheme-column spans, closed structural roles, and exact compiler-event joins. Those path and module/function identifiers cross the child boundary and remain internal. The bundle never contains source, AST values, literals, local-variable identifiers, or private-project prose. Every owned file has one complete or explicit zero-payload incomplete bundle, with hard per-file caps of 8 MiB, 500,000 AST visits, depth 256, 20,000 carriers, and 500,000 facts. The parent verifies protocol shape and caps, source ownership and regular-file/realpath containment, digest, exact file inventory, reflected carrier ownership, span containment, uniqueness, and event identity. Phase-local event IDs are canonicalized and remapped across production/test merges. A malformed test structural overlay drops only that optional overlay. Two separately reviewed roles are semantic: `runtime-mfa` joins one exact three-element tuple to its exact compiler alias event, after which the parent rereads the digest-validated span and accepts only a literal function atom; `use-dispatcher` joins the exact selector argument of dynamic `Kernel.apply/3` or `:erlang.apply/3` inside the corresponding literal-selector `__using__/1` carrier. The latter discharges only that dispatcher's existing dynamic hazard, while literal `use Module, :helper` sites remain independently compiler-confirmed. Missing, ambiguous, incomplete, or malformed evidence retains the conservative legacy behavior, and all other structural roles remain semantically inert until separately reviewed.",
  },
  {
    id: "elixir-computed-atom-local-role-flow",
    title: "Elixir computed atoms use exact local role summaries (experimental)",
    detail:
  ...
  {
    id: "elixir-public-atom-parameter-summaries",
    title: "Exact same-module public atom parameters have bounded summaries",
    detail:
  ...
  {
    id: "elixir-cross-module-public-atom-parameters",
    title: "Cross-module public atom parameters require one complete Mix boundary",
    detail:
